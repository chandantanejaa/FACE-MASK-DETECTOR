...
def detect_and_predict_mask(frame, faceNet, maskNet):
	# dimension of the frame are grabbed and a blob is created from it
	(h, w) = frame.shape[:2]
	...
	for i in range(0, detections.shape[2]):
		# calculate the probability associated with the detection
	...
	# the face location and their following predictions are returned
	return (locs, preds)
...
print("Begining of Video Stream")
...
while True:
	...
	for (box, pred) in zip(locs, preds):
		...
		#Beep if user has not covered his/her face 
		if (label=="No Mask"):
